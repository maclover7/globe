  ...
  def index
    ...
    if params[:course_id]
      if @course = current_student.courses.find_by_id(params[:course_id])
        ...
        @student_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("assignments.course_id = ?", @course.id).all
      else
        redirect_to student_root_path, notice: "Not authorized to view assignments for this course" if @course.nil?
      end
    end
    #
    @overdue_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("DATE(assignments.due_date) <= ?", Date.today).all
    @today_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("DATE(assignments.due_date) = ?", Date.today).all
    @tomorrow_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("DATE(assignments.due_date) = ?", Date.tomorrow).all
    @week_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("DATE(assignments.due_date) BETWEEN ? AND ?", Date.today.beginning_of_week, Date.today.end_of_week).all
    @next_2_weeks_assignments = current_student.student_assignments.where(completed: false).joins(:assignment).where("DATE(assignments.due_date) BETWEEN ? AND ?", Date.today.beginning_of_week, Date.today.end_of_week + 7.days).all
    @all_assignments = current_student.student_assignments.where(student_id: current_student.id, completed: false).all
  end
end
